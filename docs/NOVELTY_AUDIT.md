...
|---|---|---|---|
| 多版本选择空间 | 未经 A100/真实数据证明 | H1 按预注册阈值通过 | Source spread 稀少或 Oracle 改善 <5% |
...
| 端到端经济性 | 尚无 A100 证据 | 全成本计入后满足 `T_reuse <= 0.8 T_full` 且主要 workload 改善 | 收益 <3% 或被加载/HBM/并发干扰抵消 |
...
这个机会是否达到 SCI 一区或二区，不由措辞决定，而由 H1/H2 的 oracle gap、对 Cache-Craft 的公平优势、尾部质量保证和 A100 端到端收益共同决定。若任一关键 gate 失败，应按研究契约停止或重构，而不是增加调度模块掩盖核心假设失败。
