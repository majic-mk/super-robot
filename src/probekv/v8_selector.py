...
class TrainingFreeResidualKSelector:
    ...
    def _economic(
        ...
        return (
            shared_sunk_ms + candidate.predicted_future_upper_ms
            <= self.gamma * dense_reference_ms + 1e-12
        )
    ...
    def evaluate_checkpoint(
        ...
        *,
        ...
        if counts.correctness_eligible_k == 1:
            if self._economic(best, shared_sunk_ms, dense_reference_ms):
                return ResidualSelectionDecision(
        ...
        if counts.compared_k < 2:
            if (
                ...
                and self._economic(best, shared_sunk_ms, dense_reference_ms)
            ):
                return ResidualSelectionDecision(
                    ResidualSelectionState.LOCKED,
                    completed_depth,
        ...
        economic = self._economic(best, shared_sunk_ms, dense_reference_ms)
        common = dict(
        ...
        final = [
            ...
            and self._economic(item, shared_sunk_ms, dense_reference_ms)
        ]
        if not final:
            return ResidualSelectionDecision(
                ResidualSelectionState.ABSTAINED,
