"""Run and durably audit the real CacheBlend v6 A800 qualification matrix."""
...
def main() -> int:
    ...
    model_audit = _json(model_audit_path)
    ...
    existing = (
        _jsonl(results_path, QualificationJobResult.from_row)
        if args.resume else []
    )
    ...
    if any(not row.passed for row in existing):
        raise RuntimeError(
            "existing results contain a failed job; use a new output directory"
        )
    ...
    complete = len(existing) == len(jobs) and not stopped_on_failure
    if complete:
        validate_qualification_results(jobs, existing)
    results_hash = sha256_file(results_path) if results_path.exists() else None
    audit = {
        "schema_version": 1,
