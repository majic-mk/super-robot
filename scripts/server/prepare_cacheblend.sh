...
if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "usage: $0 /absolute/path/to/CacheBlend [cb0|probekv|probekv_closed_loop|probekv_v6_multiregion|probekv_v6_staggered_runtime|probekv_v6_prefix_hardened_runtime|probekv_v7_single_artifact_runtime|probekv_v8_training_free_residual_k|probekv_v8_schema6_joint_cfo|probekv_v8_winner_gradual_streaming]" >&2
  exit 2
fi
...
case "$mode" in
  ...
  probekv_v7_single_artifact_runtime|probekv_v8_training_free_residual_k)
    ...
    ;;
  probekv_v8_schema6_joint_cfo|probekv_v8_winner_gradual_streaming)
    patches=(
      "0001-cb0-fix-suffix-length.patch"
      "0002-probekv-segment-repair-mask.patch"
      "0003-probekv-multiregion-union-mask.patch"
