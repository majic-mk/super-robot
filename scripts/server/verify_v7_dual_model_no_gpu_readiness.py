...
from probekv.v6_server_readiness import collect_no_gpu_host
...
def main() -> int:
    ...
    manifests = {
        key: _json(getattr(args, "%s_manifest" % key)) for key in ("mistral", "qwen")
    }
    audits = {
        key: _json(getattr(args, "%s_model_audit" % key)) for key in ("mistral", "qwen")
    }
    hashes = {
        key: {
    ...
    result = evaluate_v7_no_gpu_readiness(
        ...
        _json(args.storage_audit),
        audits,
        _json(args.patch_audit),
